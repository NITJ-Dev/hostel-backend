    ...
    if (isset($data->rollno, $data->requested_rollno)) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Case 2: Initiate revoke after acceptance
                ...
                // Check if the user's roommate has already booked the room
                $stmt = $conn->prepare("SELECT COUNT(*) FROM booking WHERE requester_rollno = ? OR accepter_rollno = ?");
                if (!$stmt) {
                    throw new Exception("Prepare statement failed: " . $conn->error);
                }
                ...
                if ($rowCount > 0) {
                    http_response_code(402); // Not acceptable
                    echo json_encode(['status' => 'failure', 'message' => 'Room is already booked by roommate, can not revoke now.']);
                    exit;
                }
